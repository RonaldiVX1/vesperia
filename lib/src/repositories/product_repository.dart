...
class ProductRepository {
  ...
  Future<ProductListResponseModel> getProductList(ProductListRequestModel request) async {
    try {
  ...
  Future<RatingResponseModel> getRatingProduct(RatingRequestModel request) async {
    try {
      String endpoint = Endpoint.rating;
      final responseJson = await _client.get(
        endpoint,
        data: request,
        options: NetworkingUtil.setupNetworkOptions('Bearer ${_local.read(LocalDataKey.token)}'),
